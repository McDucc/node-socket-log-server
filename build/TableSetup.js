...
async function c(pool) {
    ...
    if (table_columns.length === 0) {
        ...
        try {
            ...
            CREATE TABLE ${env_1.env.postgres.logs_table} (
                id BIGSERIAL PRIMARY KEY,
                level SMALLINT,
                time BIGINT,
                server VARCHAR(${env_1.env.postgres.column_server_size}),
                channel VARCHAR(${env_1.env.postgres.column_channel_size}),
                message VARCHAR(256),
                data VARCHAR(4096),
                search TSVECTOR);`);
            console.log(`[${new Date().toISOString()}] Created logs table as ${env_1.env.postgres.logs_table}`);
            await client.queryString(`
            CREATE TABLE ${env_1.env.postgres.metrics_table} (
                id BIGSERIAL PRIMARY KEY,
                time BIGINT,
                server VARCHAR(${env_1.env.postgres.column_server_size}),
                cpu REAL,
                ...
                net_in REAL,
                net_out REAL);`);
            console.log(`[${new Date().toISOString()}] Created metrics table as ${env_1.env.postgres.metrics_table}`);
            await client.queryString(`
            CREATE TABLE ${env_1.env.postgres.triggers_table} (
                id BIGSERIAL PRIMARY KEY,
                name VARCHAR(128),
            ...
            BEGIN
                ...
                new.search :=
                    setweight(to_tsvector('${env_1.env.postgres.language}', new.data), 'A')
                    || setweight(to_tsvector('${env_1.env.postgres.language}', new.message), 'B');
                return new;
                END
            $$ LANGUAGE plpgsql`);
            ...
            ON ${env_1.env.postgres.logs_table} FOR EACH ROW
